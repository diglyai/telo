...
example:
	pnpm run --filter='*' build && bun runtime/nodejs/bin/cito.cjs $(EXTRA_ARGS) examples/$(EXAMPLE_NAME)

test:
	pnpm run --filter='*' build && bun runtime/nodejs/bin/cito.cjs $(EXTRA_ARGS) tests/$(EXAMPLE_NAME)
